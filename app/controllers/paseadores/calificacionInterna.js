...
function busca_paseos2() {
    ...
    ref.orderByChild("calificacion").once("value").then(snapshot => {
        ...
        for (const property in datos) {
            ...
            var uid = d["idPaseador"] || "Sin dato";
            var nombre = `${d["nombre"]} ${d["apellidopa"]} ${d["apellidoma"]}`;
            var califInterna = d["calificacionInterna"] || "sin calificacion";
            var calif = d["calificacion_final"] || 0;

            calif = Math.round(calif * 100) / 100;

            let nivel = "";
            if (califInterna === "sin calificacion") {
                nivel = "Sin nivel";
            } else if (califInterna < 48) {
                nivel = "sin nivel";
