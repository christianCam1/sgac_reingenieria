...
function obtener_perros() {
    ...
    //Obtencion de referencia a la BD y apunta a un nodo especifico
    var db = firebase.database();
    var ref = db.ref("RecuperandogQR");

    //Busca todas las entradas donde el campo "uid" es igual al valor "uid actual" y toma un snapshot ara analizar
    ref.orderByChild("uid").equalTo(uid).once("value").then(snapshot => {

        let lang = 'es-US' // you may use user's computer language: navigator.language || navigator.userLanguage
        let options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: "numeric", minute: "numeric" };

        //Almacena los valores de snapshoit en datos
        var datos = snapshot.val();

        //Extrae todas las propiedades y da formato a cada campo
        for (const property in datos) {
            ...
            //Verifica si los campos son distintos de "undefined"
            ...
            //Guarda todos los datos procesados sobre cada perro y su dueño en el siguiente objeto
            ...
            //Agrega este objeto a un arreglo
        ...
        */
        ...
        //Si se completan dos arreglos, llama a la funcion combinarArrays()
...
//Funcion para consultar perros
function consulta_perros() {

    //Instancia a firebase y acceso a la ruta "Perros" mediante un nodo hijo "uid"
    var db = firebase.database();
    var ref = db.ref("Perros").child(uid);

    //Ordena las entradas por el valor "idPerro" y hace un snapshot de los datos de esta ruta
    ref.orderByChild("idPerro").once("value").then(snapshot => {
        ...
        //Almacena en la variable "datos" el valor de snapshot
        var datos = snapshot.val();

        //Recorre las propiedades dentro del objeto datos
        for (const property in datos) {
            ...
            //Comprueba si los valores no son "undefined"
            ...
            //Crea y almacena toda la imformación en el siguiente objeto
            var informacionUsuarios = {
                idPerro: idPerro,
                nombrePerro: nombre,
                raza: raza,
            ...
            //Añade la información del objeto "informacionUsuarios" al arreglo "arrayPerros"
        ...
        arraysCompletados++
        //Si el valor de la variable anterior es igual a 2 manda a llamar a la funcion combinarArrays
        if (arraysCompletados == 2) {
...
function combinaArrays() {
    ...
    //Asigna los datos a un arreglo mediante la funcion map, con ayuda
    //del operador de propagacion y mediante que los valores de "idPerro"
    //coincidan. Si ambos arreglos tienen las mismas propiedades, el valor del segundo
    //arreglo "arrayPerros" sobreescribe al del primero "arrayRecuperandogQR"
    //El nuevo arreglo contiene propiedades combinadas de los dos arreglos
    ...
    //Llama a la función "creaTabla" y le pasa la variable "mergedData"
    creaTabla(mergedData);


}

//Inicio función creaTabla
function creaTabla(datos) {
    ...
    //Define una tabla de tipo DataTables vinculada al elemento "tablaPerros"
    ...
    //Agrega un nuevo filtro de busqueda personalizado
    ...
    //Oculta el campo de busqueda original y muesta el nuevo campo de busqueda
    oldInput.hide().after(newInput);

    //Funcion que transforma el texto ingresado en una expresión regular 
    ...
    //Recorre cada propiedad del arreglo datos que se recibio como parametro de la funcion
    for (const property in datos) {
        ...
        //Verifica que los valores sean distintos de undefined, de ser asi agrega los valores
        ...
        //Crea un nuevo arreglo "informacionUsuarios"
        var informacionUsuarios =
            ...
            [
                ...
                fecha_vencimiento,
                '<button class="transparent_button" onclick="restarVigencia(\'' + idPerro + '\')">  <img id="msjImg" src="../public/img/btnMenos.png"  width="50" height=" 50"> </button>',
                '<button class="transparent_button" onclick="sumarVigencia(\'' + idPerro + '\')">  <img id="msjImg" src="../public/img/btnMas.png"  width="50" height=" 50"> </button>',
                vigencias,
        ...
        //Agrega los valores de "informacionUsuarios" a la tabla usando
        //la API rows.add() que sirve para agregar una nueva fila a la tabla
        tabla.rows.add([informacionUsuarios]);
...
function sumarVigencia(idPerro) {
    ...
    const informacionUsuarios = [
        ...
        mergedData[objIndex].fecha_vencimiento_timestamp,
        mergedData[objIndex].fecha_vencimiento,
        `<button class="transparent_button" onclick="restarVigencia('${mergedData[objIndex].idPerro}')"> <img src="../public/img/btnMenos.png" width="50" height="50"></button>`,
        `<button class="transparent_button" onclick="sumarVigencia('${mergedData[objIndex].idPerro}')"> <img src="../public/img/btnMas.png" width="50" height="50"></button>`,
        mergedData[objIndex].vigencias,
        mergedData[objIndex].nueva_fecha_vencimiento_timestamp,
        mergedData[objIndex].nueva_fecha_vencimiento,
    ];
